...
class Logger(logging.Logger):
    def __init__(self, name="Base", sub_name="Server"):
        ...
        self.__logger__format__ = f"[{self.name}][{self.sub_name}]" \
                                  f"[%(filename)s][%(asctime)s][%(threadName)s/%(levelname)s]: %(message)s"
        ...
        global __LOGGER_MAIN__
        if __LOGGER_MAIN__ is None:
            __LOGGER_MAIN__ = self
...
def getLoggerMain():
    if __LOGGER_MAIN__ is not None:
        return __LOGGER_MAIN__
    else:
        raise TypeError("the main parentLogger has not been started yet")


if __name__ == '__main__':
    a = Logger(name="mcdr-plugin-repo")
    a.setDebug(True)
    a.debug("Test")
    a.info("Test")
    a.warning("Test")
